...
**잔여**: ① 프론트 화면(가입→코드입력→재발송) ② SES 운영 셋업(도메인 Verified Identity + DKIM/SPF + 샌드박스→프로덕션 + SSM 발신주소). SES 는 인프라 작업이라 코드 영향 없음. — SES 프로덕션 액세스 **승인 완료**(케이스 178300290200973, 2026-07-19). 샌드박스 탈출 → 검증 안 된 임의 수신자에게도 발송 가능(한도 하루 50,000통 / 초당 14통, EnforcementStatus HEALTHY). 이로써 자체 이메일 가입(D.2)이 실사용자 대상으로 완전 동작.
