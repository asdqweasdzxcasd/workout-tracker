...
**잔여**: ① 프론트 화면(가입→코드입력→재발송) ② SES 운영 셋업(도메인 Verified Identity + DKIM/SPF + 샌드박스→프로덕션 + SSM 발신주소). SES 는 인프라 작업이라 코드 영향 없음. — SES 프로덕션 액세스는 **심사 대기 중**(케이스 178300290200973, 2026-07 재심사 요청). 승인 전 샌드박스는 검증된 주소로만 발송.
